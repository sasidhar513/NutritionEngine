...
def display_output(x,y,theta,finalFoods,grams,daily_nutrients_limit_y,reqMineralList):
	food_items=list([])
	...
	requiredMineralNames=[constants.NUTRIENT_LIST[i] for i in reqMineralList]
	...
	for looper3 in range(len(finalFoods)):
		...
		reqNutrientValues=[float(food_dict[finalFoods[looper3]][y1] )*(float(weight/100)) for y1 in [x+constants.NUTRIENT_START_INDEX for x in reqMineralList]]
		outputDoubleArray.append([foodName,id,weight]+reqNutrientValues)

	outputDoubleArray.append(['-'*27,'-'*12,'-'*12]+['-'*12]*len(requiredMineralNames))
	outputDoubleArray.append(['Total','','']+regression.dot_product(x,theta).tolist()[0])
	outputDoubleArray.append(['-'*27,'-'*12,'-'*12]+['-'*12]*len(requiredMineralNames))
	outputDoubleArray.append(['Required','','']+y.tolist()[0])
	outputDoubleArray.append(['-'*27,'-'*12,'-'*12]+['-'*12]*len(requiredMineralNames))
	outputDoubleArray.append(['Difference','','']+((y-regression.dot_product(x,theta)).tolist()[0]))
	outputDoubleArray.append(['-'*27,'-'*12,'-'*12]+['-'*12]*len(requiredMineralNames))
	outputString=''
	...
	open(constants.OUTPUT_FILE,'w').write(outputString)
	

def display_output_normalized(x,y,theta,finalFoods,grams,daily_nutrients_limit_y,reqMineralList,normalizeList):
	food_items=list([])
	with codecs.open(constants.NUTRIENT_DETAILS_FILE, 'r', encoding='utf-8', errors='ignore') as fdata:
		food_items=fdata.read().split('\n')[1:]
	food_items=[food_item.split("^") for food_item in food_items]
	...
	open(constants.NORMALISED_OUPUT_FILE,'w').write(outputString)
